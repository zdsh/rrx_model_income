...
def get_credit_card_info_feature(credit_card_detail):
    ...
    feature['credit_amount_mean'] = str(df.mean()).split('\n')[0].split('    ')[1]
    feature['credit_amount_median'] = str(df.median()).split('\n')[0].split('    ')[1]
    feature['credit_amount_max'] = str(df.max()).split('\n')[0].split('    ')[1]
    feature['credit_amount_min'] = str(df.min()).split('\n')[0].split('    ')[1]
